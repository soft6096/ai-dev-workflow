...
```
需求文档
  │  （先做开发场景判定，由人确认：新项目→0.0；老代码迭代→0.5；已接入规范项目→1.1）
  ▼
0.0 项目初始化 ──→ 脚手架检查清单（从零生成工程时必做）
  │
  ▼
0.5 存量代码扫描 ──→ 老代码迭代新需求必做：
  │                ① 扫描老项目约定（包/返回体/命名/数据访问/中间件/项目级约束文件）
  │                ② 【优化决策闸门】是否按规范优化存量代码？
  │                   ├─ 是（选 java/comment/database-standards，可多选）──→ 先做存量优化 ──→ 1.1
  │                   ├─ 否（跳过）──────────────────────────────────────→ 1.1（2.1 走存量适配模式）
  │                   └─ 未明确选择 ─────────────────────────────────────→ End（流程终止）
  │                ③ 产出扫描结论 docs/0.5-存量代码扫描.md
  │
  ▼
