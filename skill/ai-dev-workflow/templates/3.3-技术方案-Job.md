...
│   └─ SQL: SELECT order_id FROM sys_order
...
└─ closeOrder(Long orderId) → int                        # 逐条关闭订单
    └─ SQL: UPDATE sys_order SET status = 'CLOSED', update_time = NOW()
...
- [ ] 查询 SQL 写全（筛选条件 + LIMIT），并在调用骨架 Mapper 内联 + 登记到"数据模型与 SQL"章节
- [ ] 核心逻辑含**调用骨架**：Job → Service → Mapper 分层 + `→` 调用标注 + 中文编号步骤 + Mapper 内联 SQL
- [ ] 失败处理明确（单条隔离 + 告警阈值）
