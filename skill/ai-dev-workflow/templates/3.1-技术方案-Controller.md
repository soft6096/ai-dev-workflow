...
> [!WARNING] 接口明细（★ 必填）
> 汇总表只给类型不够——**每一个 URL 都必须有独立小节，给出入参示例（请求体 JSON）+ 出参示例（成功 / 失败）**，禁止"只给类型不给示例"。出参示例的成功与失败必须各给一个，失败示例与汇总表错误码一一对应。
...
- [ ] 接口明细：**每个 URL 有独立入参示例（请求体 JSON）+ 出参示例（成功 / 失败）**，失败示例与错误码一一对应；禁止只给类型不给示例
- [ ] 校验规则覆盖必填/格式/长度/枚举/唯一
- [ ] 数据模型与 SQL 章节（3.0 通用章节）完整：建表 DDL（含索引/约束）+ 初始化数据 + 查询 SQL
- [ ] 权限注解 + 事务边界已写明（对照项目约束）
