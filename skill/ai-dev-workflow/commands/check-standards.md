...
1. 对下方每一项，**实际执行「标准检查指令」**（grep/ast-grep），把**命令 + 命中行（文件:行号）粘贴为证据**
2. 无命中 → 记 `✅`；命中违规 → 记 `❌`（附证据 + 严重度）
3. **HIGH ❌** → 按对应规范出处**当场补齐** → 重跑该项指令 + 新证据（已修复）；补齐后仍无法满足 → **升级人工核对**（在报告中标注"需人工核对"）
4. **INFO ❌** → 记录参考，不阻塞
5. 输出《关键规范核对报告》（逐项 ✅/❌ + 证据），结果汇总进 5.2 验收报告"关键规范落地核对表"
...
| :---: | :--- | :--- | :--- | :--- |
| 1 | OpenAPI/Swagger 支持 | `grep -n "springdoc\|knife4j" pom.xml`；`grep -rln "@Tag\|@Operation" src/main/java`；`grep -rln "@Schema" src/main/java` | 接口模块 pom 有 springdoc-openapi（BOM 管理，无手写版本）；每个 `*Controller.java` 类有 `@Tag`、方法有 `@Operation`；`*DTO/*VO.java` 字段有 `@Schema` | build-standards `dependency-standards.md` 4.6；java-code-standards `api-doc-standards.md` |
| 2 | Logback 支持 | `ls src/main/resources/logback-spring.xml`；`grep -rln "@Slf4j" src/main/java`；`grep -rn "System.out" src/main/java` | `logback-spring.xml` 存在（控制台+滚动文件+环境级 level）；Controller/ServiceImpl 类均有 `@Slf4j`；无 System.out / 字符串拼接日志 | java-code-standards `04-logging-standards.md` |
| 3 | SQL 全在 XML | `grep -rn "@Select\|@Insert\|@Update\|@Delete\|<script>" src/main/java`（**应为空**）；`ls src/main/resources/mapper/*.xml` | Java 无注解 SQL；手写 SQL 全在 `resources/mapper/*.xml`，namespace 与 Mapper 接口全限定名一致；接口方法均有 XML/BaseMapper 实现 | database-standards `mybatis-plus/mybatis-xml-standards.md` |
...
| 6 | JSON 入参/出参产物 | `ls docs/*/3.4-接口清单-前后端通用.md`；`grep -n "入参\|出参" docs/*/3.4-接口清单-前后端通用.md` | `3.4-接口清单-前后端通用.md` 已生成；每接口有 URL+方法+**JSON 入参示例**+**JSON 出参示例（成功/失败）**；与技术方案一致（纯后端模块标注跳过） | ai-dev-workflow `templates/3.4-接口清单-前后端通用.md` |
...
| 10 | 统一返回体 | `grep -rn "Map<" src/main/java/*/controller/*.java`；`grep -rln "class R\b\|class Result\b" src/main/java` | Controller 方法返回 `Response<T>`/`PageResult<T>`，无 Map 裸返回；无 `R`/`Result`/`ApiResponse` 类 | java-code-standards `01-naming-standards.md` / `controller-standards.md` |
...
- 报告对话输出 + 关键结论（6 项核心 HIGH）汇总进 5.2 验收报告「关键规范落地核对表」
