...
**适用范围**：从零生成/初始化 Spring Boot 工程（含 AI 一次生成整个项目）。**在既有项目里迭代新需求则跳过本节**：老代码基础上迭代 → 先走 0.5 存量代码扫描与约束适配；已有规范项目里开发新模块 → 直接走 1.1。
...
1. **先扫描老代码**（提醒并执行，按以下维度逐项记录证据——文件路径/配置项）：
   ...
   - **既有约定**：老项目 AGENTS.md / README / 已有接口文档里的约定
...
2. **询问人：是否按规范优化一版存量代码？**（可多选，也可跳过）：
   - `java-code-standards`（AI 开发代码规范）
   - `comment-standards`（AI 开发注释规范）
   - `database-standards`（AI 开发 SQL 规范）
   - `跳过`：不优化存量代码，只保证新需求代码与老项目一致
   - 选择优化 → 先按所选规范对存量代码做一版优化（改到哪规范到哪，不追求一步全量），完成后更新扫描结论，再进入 1.1
...
用户说"按流程开发 XX 模块"时，按下方流程顺序执行（老代码迭代先走 0.5，其余从 1.1 起）。**每步产物必须给人确认后才能进入下一步**（见下方"中间产物确认闸门"）。中途用户可要求跳到任一 `/命令`。
...
- **输入**：需求文档
- **动作**：**先过滤非 Java 服务端需求**（前端页面/移动端/小程序/其他语言服务端等 → 不拆功能项，记入"已过滤"清单，不进入本流程开发范围）；仅对 Java 服务端需求（Controller/Listener/Job/无入口）拆功能项
- **输出**：功能清单总览文件（表格），每项含：类型（Controller/Listener/Job）、优先级、依赖、**验收标准**（可验证，不能写"实现 XX 功能"）
- **完成标准**：每项有类型 + 可验证验收标准 + 依赖标注；非 Java 服务端需求已过滤并记录
...
> [!NOTE] 约束的生效范围
> - **标准模式（新项目）**：2.1 生成后**同步写入项目根 `AGENTS.md`**（不存在则创建，或追加到已有 AGENTS.md 规范章节），成为整个项目的长期规范——后续所有模块开发都遵守，不限于本次。模块目录内的 `2.1-项目约束.md` 是本模块的产物副本（本次开发引用）。
