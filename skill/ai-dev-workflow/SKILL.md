...
- **兜底闸门（关键规范自动核对）**：编码收尾运行 `/check-standards`——用 grep/ast-grep 实际扫描 12 项 HIGH + 代码规范组（构造器注入/分层边界/Entity 不暴露/异常处理/命名/公共组件复用）+ 注释组（类/方法 Javadoc 覆盖率/步骤注释/禁翻译式），每项附证据；**先判模式：选型敏感项（接口文档/日志框架/数据访问层/返回体）新项目按 2.1 人确认选型判定、老项目按 0.5 扫描约定判定**，其余按规范；**未执行到位项（含 INFO，不分重要与否）先向用户确认是否补齐（人确认后才动手），仍 ❌ 升级人工核对**；5.2 验收同步复核（判定标准见 `commands/check-standards.md` + `templates/5.1-编码指令.md`"关于关键规范核对"）
