...
11. **收尾必过「关键规范自动核对」闸门**（命令见 `commands/check-standards.md`）：运行 check-standards 核对 12 项 HIGH + 代码规范组 C1-C6 + 注释组 N1-N4（含标准 grep/ast-grep 指令），每项**附实际执行证据（文件:行号）**；**未执行到位项 → 先向用户展示证据、确认"是否按规范补齐" → 用户确认后才执行补齐** → 重跑该项 + 新证据 → 仍 ❌ → 升级人工核对；禁止"没做就声称做了"、禁止未经用户确认自动改代码
```
